...
void int2float(const uint8_t* recv_data, volatile float* d_float)
{
  uint8_t* d_int;
...
void OnDataRecv(const uint8_t *mac_addr, const uint8_t *recv_data, int data_len) 
{
  ...
  int2float(&recv_data[0], &Stick[RUDDER]);
  int2float(&recv_data[4], &Stick[THROTTLE]);
  int2float(&recv_data[8], &Stick[AILERON]);
  int2float(&recv_data[12], &Stick[ELEVATOR]);
#if 0
  d_int = (uint8_t*)&d_float;
